...
from figures import FIG, C_OBS, C_FIT, C_TRUE, C_BASE, panel_letter

NUIS = [("clean", "clean"), ("texture", "+ texture"),
        ("texture+stain", "+ staining"), ("+warp", "+ deformation"),
        ("+sheet_damage", "+ sheet losses")]
...
def main(out=f"{FIG}/fig8_ablation.png"):
    ...
    fig, ax = plt.subplots(1, 2, figsize=(7.2, 3.1),
                           gridspec_kw=dict(width_ratios=[1.30, 1]))
    ...
    ax[0].text(len(keys) - 0.55, at + 0.004, "true rate", fontsize=6.5,
               ha="right", color="#555")
    ax[0].errorbar(range(len(keys)), m, yerr=e, fmt="o-", color=C_FIT, ms=5,
                   lw=1.3, capsize=2.5)
    ax[0].set_xticks(range(len(keys)))
    ax[0].set_xticklabels(labs, fontsize=6.8, rotation=20, ha="right")
    ax[0].set_ylabel("recovered rate a (mm per century)")
    ax[0].set_ylim(0, at * 1.35)
    ax[0].set_title("realistic acquisition noise does not spoil it;\n"
                    "the cleanest images are the worst case", fontsize=8, pad=6)
    panel_letter(ax[0], "a", dx=-0.13, dy=1.21)
    ...
    ax[1].set_ylim(0, 2.95)
    ax[1].legend(frameon=False, fontsize=6.4, loc="upper right",
                 bbox_to_anchor=(1.0, 1.0), handlelength=1.4)
    ax[1].set_title("four impressions is the threshold", fontsize=8, pad=6)
    panel_letter(ax[1], "b", dx=-0.22, dy=1.21)
    fig.tight_layout(rect=(0, 0, 1, 0.94)); fig.savefig(out, dpi=300); plt.close(fig)
    print("wrote", out)
