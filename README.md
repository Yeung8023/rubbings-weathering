...
**版面检测**：`src/figcheck.py` 渲染每张图后读取所有绘图对象的 bounding box，
程序化判定七类缺陷：

1. 文字超出画布  2. 文字互相重叠  3. 文字压在曲线/柱子/误差棒上
4. 文字溢出所属方框  5. 标注侵入相邻面板
6. **方框互相重叠或间距过小**（模块块、图像框、虚线框两两比对）
7. **文字压在图像上**

已排除的误报来源：`axhline`/`axvspan` 等混合坐标参考线；视野外的刻度标签；
`annotate` 包围盒里的引线箭头（matplotlib 会把箭头并进文字的 bbox）。
故意重叠的装饰件（错位平面、骑在框线上的胶囊标题牌、紧排的字帖网格）
用 `set_gid("deco")` 标注后跳过。当前 **9 张图 0 问题**。
